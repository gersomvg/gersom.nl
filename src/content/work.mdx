...
A non-exhaustive list of languages, tools and frameworks I’ve recently worked with: `JavaScript` `TypeScript` `React` `React Native` `Next` `Gatsby` `Expo` `Node` `Express` `Postgres` `Objection` `Knex` `Redux` `Styled-Components` `Emotion` `Slate`.
